...
export class BrowserWordRecognizer {
  ...
  finish(): void {
    ...
    if (!this.recognition) {
      this.finishing = false;
      this.callbacks.onState('idle', '言灵已收束');
      return;
    }
    this.callbacks.onState('processing', '正在提交最终转写');
    try {
      this.recognition.stop();
    } catch {
      this.handleEnd(this.recognition);
    }
  }
  ...
  private handleEnd(recognition: BrowserSpeechRecognition): void {
    ...
    this.detach(recognition);
    this.recognition = undefined;
    const completedUtterance = this.finishing || this.deliveredFinal;
    if (this.finishing && !this.deliveredFinal && this.lastPartialTranscript) {
      this.deliveredFinal = true;
      this.callbacks.onFinal({
    ...
    if (!this.shouldListen || this.destroyed) {
      if (!this.destroyed && this.failures === 0 && !completedUtterance) {
